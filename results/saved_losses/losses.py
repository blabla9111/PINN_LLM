...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    time_penalty = 0.1  # penalty for incorrect time dynamics
    aggregation_func = torch.mean
    norm_func = torch.square

    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(norm_func(I_hat - I_pred)) + time_penalty * aggregation_func(norm_func((I_pred - I_hat) / (I_pred_last - I_hat)))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    regul = 0.8    
    last_infected_penalty = 0.05
    time_penalty = 0.2  # Added time-based penalty
    ...
    loss = regul * (term1 + term2 + term3 + term4) + \
           (1 - regul) * (term5 + term6 + term7 + term8) + \
           time_penalty * torch.mean(norm_func(f1[:, 1:] - f1[:, :-1])) + \
           last_infected_penalty * norm_func(I_pred_last-0)
    return loss
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    time_penalty = 0.1  # new time-based penalty
    ...
    loss = regul * (term1 + term2 + term3 + term4) + \
           (1 - regul) * (term5 + term6 + term7 + term8) + \
           (time_penalty * 60 / (60 + 30)) * norm_func(I_pred_last) + \
           last_infected_penalty * norm_func(I_pred_last-0)
    return loss
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    delayed_immunity_penalty = 0.2  # penalty for delayed immune response
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + delayed_immunity_penalty * (R_hat - I_hat)  # added penalty for delayed immune response
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term6 = aggregation_func(norm_func(f2))
    term7 = aggregation_func(norm_func(f3))
    term8 = aggregation_func(torch.norm(f4))  # changed f4 from norm_func to torch.norm

    loss = regul * (term1 + term2 + term3 + term4) + \
        (1 - regul) * (term5 + term6 + term7 + term8) + \
        last_infected_penalty * norm_func(I_pred_last-0)
    return loss.unsqueeze(0)  # changed return type to scalar
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    # Remove unsqueeze(0) to make loss a scalar
    return loss
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    delayed_response_penalty = 0.05  # new penalty to account for delayed immune response
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + delayed_response_penalty * norm_func(R_hat - I_pred)  # add penalty to term4
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + delayed_response_penalty * norm_func(R_hat - I_pred)  
    ...
    loss = regul * (term1 + term2 + term3 + term4) + \
          (1 - regul) * (term5 + term6 + term7 + term8) + \
          last_infected_penalty * norm_func(I_pred_last.float()).unsqueeze(0) # ensure I_pred_last is a tensor with a single value
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    delayed_response_penalty = 0.05  # Removed this from term4 as it was causing the error
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))  # Removed delayed_response_penalty
    ...
    loss = regul * (term1 + term2 + term3 + term4) + \
          (1 - regul) * (term5 + term6 + term7 + term8) + \
          last_infected_penalty * norm_func(I_pred_last.float())
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    time_penalty_coeff = 0.1  # introduce a new parameter for time-based penalty
    
    aggregation_func = torch.mean
    norm_func = torch.square

    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(norm_func(I_hat - I_pred))  # only this term should be changed
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
    ...
    # introduce time-based penalty for term2
    time_penalty = time_penalty_coeff * norm_func((I_pred_last - I_pred) / (1e-9 + torch.abs(I_pred - I_pred_last)))  # avoid division by zero
    term2 = aggregation_func(norm_func(I_hat - I_pred)) + time_penalty  # add time-based penalty to term2
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred)) + \
            time_penalty_coeff * aggregation_func(norm_func((I_pred_last - I_pred) / (1e-9 + torch.abs(I_pred - I_pred_last))))  # avoid division by zero

    loss = regul * (term1 + term2 + term3 + term4) + \
           (1 - regul) * (term5 + term6 + term7 + term8) + \
           last_infected_penalty * aggregation_func(norm_func(I_pred_last)) # ensure aggregation for last_infected_penalty
    return loss
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(norm_func(I_hat - I_pred))  # add an offset to the desired peak day
    term2 = aggregation_func(norm_func((I_hat - I_pred) + torch.tensor([10])))  # adjust the offset as needed
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(norm_func(I_hat - I_pred + (I_pred_last - I_hat) * 20 / 20))  # add offset to the desired peak day
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    vaccination_rate_penalty = 0.1
    vaccination_rate_target = 0.7  # 70% vaccination rate
    vaccination_duration = 30  # days
    ...
    # Vaccination rate penalty term
    vaccination_rate = 1 - (I_pred / (S_pred + I_pred + R_pred + D_pred))  # vaccination rate calculation
    vaccination_rate_term = norm_func(vaccination_rate - vaccination_rate_target)  # calculate penalty term
    vaccination_rate_term = aggregation_func(vaccination_rate_term)  # apply aggregation function
    vaccination_rate_term = vaccination_rate_penalty * vaccination_rate_term  # apply penalty weight

    loss = regul * (term1 + term2 + term3 + term4) + \
          (1 - regul) * (term5 + term6 + term7 + term8) + \
          last_infected_penalty * norm_func(I_pred_last-0) + \
          vaccination_rate_term  # add vaccination rate penalty term
    return loss
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    regul = 0.8
    last_infected_penalty = 0.05
    lockdown_inertia = 5  # added parameter for lockdown inertia
    
    aggregation_func = torch.mean
    norm_func = torch.square
    
    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(norm_func(I_hat - I_pred)) * (1 - (I_pred_last / (I_pred_last + 1)) ** lockdown_inertia)  # modified term2 to include lockdown effect
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - (I_hat + R_hat_prev))) # Modified term4 to account for delayed immune response
    ...
    loss = regul * (term1 + term2 + term3 + term4) + \
        (1 - regul) * (term5 + term6 + term7 + term8) + \
        last_infected_penalty * norm_func(I_pred_last-0) + \
        delayed_response_penalty * norm_func(R_hat - (I_hat + R_hat_prev)) # Additional penalty to account for expert's comment
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    smoothness_penalty = 0.1  # New penalty to add
    ...
    term2_smooth = aggregation_func(norm_func(I_hat[:-1] - I_pred[:-1]))  # Smoothness penalty on I_pred

    loss = regul * (term1 + term2 + term3 + term4) + \
        (1 - regul) * (term5 + term6 + term7 + term8) + \
        last_infected_penalty * norm_func(I_pred_last-0) + \
        smoothness_penalty * term2_smooth  # Add smoothness penalty to term2
    return loss
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    tail_penalty = 0.01  # Add a penalty to slow down the decline in the infected population
    
    aggregation_func = torch.mean
    norm_func = torch.square

    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(norm_func(I_hat - I_pred)) + tail_penalty * aggregation_func(norm_func(I_pred))  # Add the tail penalty to term2
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    trans_rate_reduction = 0.1  # Parameter to adjust mask effectiveness
    contact_rate_reduction = 0.2  # Parameter to adjust social distancing effectiveness

    aggregation_func = torch.mean
    norm_func = torch.square

    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(norm_func(I_hat - (1 - trans_rate_reduction) * I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    mask_transmission_reduction = 0.2  # parameter to adjust the effect of masks on transmission
    social_distancing_reduction = 0.1  # parameter to adjust the effect of social distancing on transmission
    vaccination_reduction = 0.05  # parameter to adjust the effect of vaccination on transmission

    aggregation_func = torch.mean
    norm_func = torch.square

    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(norm_func(I_hat - I_pred)) * (1 - mask_transmission_reduction - social_distancing_reduction - vaccination_reduction)
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    mask_reduction_factor = 0.7  # parameter to adjust the effect of masks
    last_infected_penalty = 0.05
    ...
    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(norm_func(I_hat - mask_reduction_factor * I_pred))  # adjust term2 to consider masks
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    lockdown_inertia = 7 # adjust lockdown inertia to 7 days
    lockdown_weight = 0.2 # introduce a new parameter to weigh the lockdown term
    early_lockdown_term = (1 - regul) * lockdown_weight * torch.mean(torch.square(I_hat[:-lockdown_inertia] - I_pred[:-lockdown_inertia]))
    ...
    loss = regul * (term1 + term2 + term3 + term4) + \
        (1 - regul) * (term5 + term6 + term7 + term8 + early_lockdown_term) + \
        last_infected_penalty * norm_func(I_pred_last-0)
    return loss
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    lock_down_factor = 0.2  # Introduced parameter to affect the lockdown measures
    lock_down_inertia = 5  # Introduced parameter to represent the inertia of lockdown measures

    aggregation_func = torch.mean
    norm_func = torch.square

    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(norm_func(I_hat - I_pred))  # Only this term can be changed
    term2 = term2 + lock_down_factor * torch.square(I_pred_last / (I_pred_last + 1))  # Adjusted term to consider lockdown measures
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(0.7 * norm_func(I_hat - I_pred) + 0.3 * norm_func(I_pred - I_pred_last))  # Introduced measure: reduction of the infected population growth rate due to lockdown
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    regul = 0.8
    last_infected_penalty = 0.05
    delayed_response_penalty = 0.1  # New penalty for delayed immune response
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - (I_hat + D_hat) / 2))  # Modified term4
    ...
    loss = regul * (term1 + term2 + term3 + term4) + \
           (1 - regul) * (term5 + term6 + term7 + term8) + \
           last_infected_penalty * norm_func(I_pred_last-0) + \
           delayed_response_penalty * norm_func(R_hat - I_hat)  # New term for delayed immune response
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    loss = regul * (term1 + term2 + term3 + term4) + \
           (1 - regul) * (term5 + term6 + term7 + term8) + \
           last_infected_penalty * norm_func(I_pred_last) + \
           delayed_response_penalty * norm_func(R_hat - I_hat)  # Modified term for delayed immune response
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - (I_hat + D_hat)))  # Modified term4 to avoid division by zero
    ...
    loss = regul * (term1 + term2 + term3 + term4) + \
           (1 - regul) * (term5 + term6 + term7 + term8) + \
           last_infected_penalty * norm_func(I_pred_last) + \
           delayed_response_penalty * norm_func(R_hat - I_hat)
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    delayed_immune_response_penalty = 0.1
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + delayed_immune_response_penalty * norm_func(R_hat - 0.7 * I_hat) # modified term4 with delayed immune response penalty
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    # Corrected term4 to match the shapes of f3 and R_hat
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + delayed_immune_response_penalty * norm_func(R_hat - 0.7 * I_hat)
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    # Corrected terms5, 6, 7, 8 to match the shapes of f1, f2, f3, f4
    term5 = aggregation_func(norm_func(f1))  # f1 shape: torch.Size([n, m])
    term6 = aggregation_func(norm_func(f2))  # f2 shape: torch.Size([n, m])
    term7 = aggregation_func(norm_func(f3))  # f3 shape: torch.Size([n])
    term8 = aggregation_func(norm_func(f4))  # f4 shape: torch.Size([n])

    loss = regul * (term1 + term2 + term3 + term4) + \
           (1 - regul) * (term5 + term6 + aggregation_func(norm_func(f3)), term7) + \
           last_infected_penalty * norm_func(I_pred_last)
    
    # Corrected the calculation of term7 to be compatible with aggregation_func
    return loss
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    smoothness_penalty = 0.1  # added penalty for smoothness of infected population curve

    aggregation_func = torch.mean
    norm_func = torch.square

    term1 = aggregation_func(norm_func(S_hat - S_pred))
    term2 = aggregation_func(norm_func(I_hat - I_pred)) + smoothness_penalty * aggregation_func(norm_func(f2))  # added penalty for smoothness of infected population curve
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred))
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    delayed_response_penalty = 0.1  # add penalty to account for delayed immune response
    last_infected_penalty = 0.05
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + delayed_response_penalty * torch.square(R_hat - I_hat)  # add penalty to term4
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + recovered_penalty * norm_func(R_hat - R_pred)
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    early_recovered_penalty = 0.1  # New penalty term
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + early_recovered_penalty * (R_pred - 0)  # Modified term4
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + early_recovered_penalty * (R_pred ** 2)  # Modified term4
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + early_recovered_penalty * torch.mean(torch.square(R_hat - R_pred))[:4]  # add penalty to the first 4 elements
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + \
            early_recovered_penalty * torch.mean(torch.square(R_hat - R_pred))[:4]  # add penalty to the first 4 elements
    ...
    loss = regul * (term1 + term2 + term3 + term4) + \
           (1 - regul) * (term5 + term6 + term7 + term8) + \
           last_infected_penalty * torch.square(I_pred_last) # modified to avoid 0-d tensor issue
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    day_penalty = 0.5  # New penalty term for recovered population in early days
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + day_penalty * torch.max(torch.zeros_like(R_hat), torch.relu(torch.ones_like(R_hat) - (R_hat / R_hat.max()) * 0.05))  # New term for recovered population in early days
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term5 = aggregation_func(norm_func(f1))
    term6 = aggregation_func(norm_func(f2))
    term7 = aggregation_func(norm_func(f3))  # shape: torch.Size([n])
    term8 = aggregation_func(norm_func(f4))  # shape: torch.Size([n])
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + \
        day_penalty * torch.max(torch.zeros_like(R_hat), torch.relu(torch.ones_like(R_hat) - (R_hat / R_hat.max()) * 0.05))  # New term for recovered population in early days

    term5 = aggregation_func(norm_func(f1))
    term6 = aggregation_func(norm_func(f2))
    term7 = aggregation_func(torch.mean(norm_func(f3)))  # shape: torch.Size([n])
    term8 = aggregation_func(torch.mean(norm_func(f4)))  # shape: torch.Size([n])
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    regul = 0.8
    last_infected_penalty = 0.05
    first_days_penalty = 0.1  # added penalty for recovered population in the first days
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + first_days_penalty * norm_func(R_hat[:3])  # added penalty for recovered population in the first days
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + early_recovery_penalty * torch.max(torch.zeros_like(R_hat), R_hat - 10)
...
def loss_dinn(S_hat, S_pred, I_hat, I_pred, D_hat, D_pred, R_hat, R_pred, f1, f2, f3, f4, I_pred_last):
    ...
    term2 = aggregation_func(norm_func(I_hat - I_pred))
    term3 = aggregation_func(norm_func(D_hat - D_pred))
    term4 = aggregation_func(norm_func(R_hat - R_pred)) + initial_recovered_penalty * (R_pred - R_hat)
