...
def generate_model_page():
    ...
    # # return
    experiment_logger = TensorBoardExperimentLogger(log_dir="./runs/CUSTOM_TRAINING")
    
    # ВАЖНО потом убрать!!!!
    EXPERT_COMMENT = "During the peak, the number of cases should be more"
    experiment_logger.save_expert_comment(EXPERT_COMMENT)

    response = supabase.storage.from_("PINN_LLM_STORAGE").download("loss/loss_dinn_CUSTOM_TRAINING.py")
    code = get_loss_func_as_str(loss_filepath)
    loss_after = code
    ...
    response = supabase.storage.from_("PINN_LLM_STORAGE").download("dinn_cuda_NEW_PINN_CUSTOM_TRAINING.pth")
